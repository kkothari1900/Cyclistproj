...
annual_mem_2 <- data.frame(annual_mem$start_lng,annual_mem$start_lat,annual_mem$end_lng,annual_mem$end_lat)
...
library(geosphere)
annual_mem %>% rowwise%>% mutate(distance=distm(x=c(start_lng,start_lat),y=c(end_lng,end_lat),fun = distGeo))
...
casual_mem %>% rowwise %>% mutate(distance = distm(x = c(start_lng,start_lat),y=c(end_lng,end_lat),fun = distGeo))

# Conversion of Distance units
library(measurements)
conv_unit(annual_mem)


```

# Distance Explained 
The Distm() function is used to calculate the distance of the start (longitude, latitude) and end (longitude, latitude) coordinates.When this function is used, it automatically calculated in meters. There will be a conversion of metrics using the measurements library from meters to meters to ** miles or kilometers. 
**NEWUPDATE**
Using fun= distGeo, This is Distance on an ellipsoid (the geodesic). This function is an accurate estimate of the shortest distance between two points on an ellipsoid (default is WGS84 ellpsoid) 

Geodesic: The shortest path between two points on an ellipsoid 
```{r}
