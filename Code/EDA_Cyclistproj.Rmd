...
library(geosphere)
annual_mem %>% rowwise()%>% mutate(distance=distm(x=c(start_lng,start_lat),y=c(end_lng,end_lat),fun = distHaversine)[,1]/1609)
...
casual_mem %>% rowwise %>% mutate(distance = distm(x = c(start_lng,start_lat),y=c(end_lng,end_lat),fun = distHaversine)[,1]/1609)
